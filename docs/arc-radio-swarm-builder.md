...
The unpushed ARC and ArcUI implementation remains isolated on
`codex/42-complete-swarm-onboarding`. The AVIAN-owned documentation and
read-only validation harness have been ported onto
`codex/avian-arc-main-compat-20260827`, based on the newest AVIAN main. The
existing automated ARC evidence includes 382 passing dev-bridge library tests
(with one ignored), 10 focused swarm API tests, and 26 focused ArcUI tests.
Coverage includes strict MAC identity parsing, expected-size enforcement,
reload recovery, the complete no-change onboarding path, keyboard modal
behavior, destructive-action confirmation, and proof that definition
deletion/removal never invokes the CHUD apply endpoint. The ArcUI production
build, focused lint, and strict OpenSpec validation also pass. This evidence
validates the software workflow only; it is not radio-in-the-loop or
RF-topology validation.

Unmodified ARC main does not yet contain the radio routes or UI components for
this workflow. AVIAN can coexist with ARC main, but the guided workflow remains
unavailable until the ticketed ARC integration is reviewed and merged. See
[ARC main compatibility](arc-main-compatibility.md).
